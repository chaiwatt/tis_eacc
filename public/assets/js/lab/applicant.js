...
$(document).on('click', '#create_address', function(e) {
    e.preventDefault();
    // ดึงค่าจากฟิลด์ต่างๆ
    let purposeValue = $('input[name="purpose"]:checked').val();
    if(purposeValue == 2)
    {
        alert('ไม่สามารถเพิ่มรายการสำหรับต่ออายุใบรับรอง');
        return;
    }
...
$(document).on('click', '.add-lab-scope', function(e) {
    ...
    if(purposeValue == undefined)
        {
            alert('กรุณาเลือกวัตถุประสงค์ในการยื่นคำขอ');
            return;
        }
    
    if(purposeValue == 2)
    {
        alert('ไม่สามารถแก้ไขขอบข่ายสำหรับต่ออายุใบรับรอง');
        return;
    }
    e.preventDefault();
...
function renderTestScopeWithParameterTable() {
    ...
    // ตรวจสอบค่า
    // if (purposeValue) {
    //     console.log("Value ของ purpose ที่ถูกเลือกคือ: " + purposeValue);
    // } else {
    //     console.log("ยังไม่ได้เลือกค่า purpose");
    // }
...
$(document).on('change', '#select_certified', function() {
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            var labCalRequest = result.labCalRequest;
