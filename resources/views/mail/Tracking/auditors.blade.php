   ...
   <p style="font-size:30px;"> 
         ...
         @if($authorities != '-')
           @foreach($authorities as $key4 => $item)
                ...
                <p  class="indent50"> 
                     ตามที่สำนักงานมาตรฐานผลิตภัณฑ์อุตสาหกรรมได้แจ้งรายละเอียดการแต่งตั้งคณะผู้ตรวจประเมิน ของ  {{  !empty($certi->name) ?   $certi->name   :  ''  }} 
                     ...
                     โดย   {{  !empty($certi->name) ?  $certi->name :  ''  }}  
                     มีความเห็นดังนี้
                </p>
                ...
                <p  class="indent50">  
                    ...
                    @foreach($attachs as $files)
                        <p class="indent50">     
                            {{  @$files->caption  }}
                           @if(!empty($files->url))
                                    <a href="{{url('funtions/get-view/'.$files->url.'/'.( !empty($files->filename) ? $files->filename :  basename($files->url)  ))}}" target="_blank">
                                        {!! basename($files->url)  ?? '' !!}
                                    </a>
                            @endif
                        </p>
