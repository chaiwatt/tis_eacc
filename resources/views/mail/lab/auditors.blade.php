   ...
   <p style="font-size:30px;"> 
        ...
        <p><b>เรื่อง การแต่งตั้งคณะผู้ตรวจประเมิน</b></p>  
         @if($authorities != '-')
           ...
           <p  class="indent50">   
                ตามที่สำนักงานมาตรฐานผลิตภัณฑ์อุตสาหกรรมได้แจ้งรายละเอียดการแต่งตั้งคณะผู้ตรวจประเมิน ของ  {{  !empty($certi_Lab->BelongsInformation->name) ?  $certi_Lab->BelongsInformation->name  :  ''  }} 
                ...
                โดย   {{  !empty($certi_Lab->name) ?  $certi_Lab->name :  ''  }}   มีความเห็นดังนี้ 
           </p>
               ...
               @php 
                ...
                <p>   
                    ...
                    @foreach($attachs as $files)
                        <p class="indent50">     
                            {{  @$files->file_desc  }}
                            <a href="{{url('certify/check/file_client/'.$files->file.'/'.( !empty($files->attachs_client_name) ? $item1->attachs_client_name : 'null' ))}}" target="_blank">
