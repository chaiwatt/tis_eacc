        ...
        <p><b>เรื่อง การแต่งตั้งคณะผู้ตรวจประเมิน</b></p>  
         ...
         @if($authorities != '-')
           @foreach($authorities as $key4 => $item)
             <p  class="indent50">  
                ตามที่สำนักงานมาตรฐานผลิตภัณฑ์อุตสาหกรรมได้แจ้งรายละเอียดการแต่งตั้งคณะผู้ตรวจประเมิน ของ {{  !empty($certi_ib->name) ? $certi_ib->name :  ''  }} 
                ...
                โดย   {{  !empty($certi_ib->name) ?   $certi_ib->name  :  ''  }}  มีความเห็นดังนี้
             </p>
               ...
               @php 
                ...
                <p>   
                    ...
                    @foreach($attachs as $files)
                        <p class="indent50">     
                            {{  @$files->file_desc  }}
                            <a href="{{ url('certify/check/files_ib/'.$files->file) }}"   target="_blank"> 
