...
<div class="form-group {{ $errors->has('applicant_20bis_id') ? 'has-error' : ''}}">
  {!! Form::label('applicant_20bis_id', 'เลขที่คำขออ้างอิง:', ['class' => 'col-md-4 control-label required']) !!}
  <div class="col-md-6">
    {!! Form::select('applicant_20bis_id',
                      $applicant_20biss->pluck('ref_no', 'id'),
                      null,
                      ['class' => 'select2 form-control', 'required' => 'required', 'placeholder'=>'- เลือกเลขที่คำขอ -'])
    !!}
    {!! $errors->first('applicant_20bis_id', '<p class="help-block">:message</p>') !!}
  </div>
</div>
...
<div class="form-group">
  ...
  <div class="col-md-3">
    <input type="text" class="form-control" id="show-title" disabled="disabled" value="{{ @$applicant_20biss->pluck('title', 'id')[@$volume_20bi->applicant_20bis_id] }}">
  </div>
  <div class="col-md-2">
  ...
  <div class="col-md-3">
    <input type="text" class="form-control" id="show-date" disabled="disabled"
           value="{{ HP::DateThai(@$applicant_20biss->pluck('start_date', 'id')[@$volume_20bi->applicant_20bis_id]) }} - {{ HP::DateThai(@$applicant_20biss->pluck('end_date', 'id')[@$volume_20bi->applicant_20bis_id]) }}">
  </div>
</div>
...
<table class="table color-bordered-table primary-bordered-table">
    ...
    <tbody id="table-body">
      ...
      @foreach ($applicant_20bis_details as $key => $item)

        @php
          $sum_informed = 0;
          foreach ($item->informed as $informed) {
            if($informed->volume_20bis_id!=$volume_20bi->id){
        ...
        <tr>
          ...
          <td>
            <div class="checkbox checkbox-success">
              <input id="detail-2" name="detail_id[{{ $item->id }}]" class="detail-item" type="checkbox" value="1" @if(array_key_exists($item->id, $volume_20bis_details)) checked="checked" @endif>
              <label for="detail-2"> ผลิต </label>
            </div>
          </td>
          <td>
            <input class="form-control quantity" name="quantity[{{ $item->id }}]" type="number" @if(array_key_exists($item->id, $volume_20bis_details)) value="{{ $volume_20bis_details[$item->id] }}" @else disabled="disabled" @endif step="0.01" max="9999999999.99">
          </td>
...
<div class="form-group">
  <div class="col-md-offset-4 col-md-4">
    ...
    <a class="btn btn-default" href="{{url('/esurv/volume_20bis')}}">
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    check_max_size_file();
    var applicant_20biss = JSON.parse('{!! json_encode($applicant_20biss->toArray()) !!}'); //ข้อมูลคำขอทำ

    //เมื่อเลือกเลขที่อ้างอิง
    $('#applicant_20bis_id').change(function(event) {
      ...
      if(id!=''){
        ...
        //แสดงข้อมูลคำขอ
        $(applicant_20biss).each(function(index, item) {
          if(item.id==id){
        ...
        $.ajax("{{ url('esurv/applicant_20bis/detail-item') }}/" + id)
         .done(function(data) {
            ...
            if(data.length>0){
              ...
              $(data).each(function(index, item) {
                ...
                $(item.informed).each(function(index, informed) {
                  if(informed.volume_20bis_id!={{(int)@$volume_20bi->id}}){
