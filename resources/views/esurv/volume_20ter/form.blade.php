...
<div class="form-group {{ $errors->has('applicant_20ter_id') ? 'has-error' : ''}}">
  {!! Form::label('applicant_20ter_id', 'เลขที่คำขออ้างอิง:', ['class' => 'col-md-4 control-label required']) !!}
  <div class="col-md-6">
    {!! Form::select('applicant_20ter_id',
                      $applicant_20ters->pluck('ref_no', 'id'),
                      null,
                      ['class' => 'select2 form-control', 'required' => 'required', 'placeholder'=>'- เลือกเลขที่คำขอ -'])
    !!}
    {!! $errors->first('applicant_20ter_id', '<p class="help-block">:message</p>') !!}
  </div>
</div>
...
<div class="form-group">
  ...
  <div class="col-md-3">
    <input type="text" class="form-control" id="show-title" disabled="disabled" value="{{ @$applicant_20ters->pluck('title', 'id')[@$volume_20ter->applicant_20ter_id] }}">
  </div>
  <div class="col-md-2">
  ...
  <div class="col-md-3">
    <input type="text" class="form-control" id="show-date" disabled="disabled"
           value="{{ HP::DateThai(@$applicant_20ters->pluck('start_date', 'id')[@$volume_20ter->applicant_20ter_id]) }} - {{ HP::DateThai(@$applicant_20ters->pluck('end_date', 'id')[@$volume_20ter->applicant_20ter_id]) }}
                 ">
  </div>
</div>
...
<table class="table color-bordered-table primary-bordered-table">
    <thead>
      <tr>
          ...
          <th class="text-center">ปริมาณการส่งออก</th>
      ...
      @foreach ($applicant_20ter_details as $key => $item)
        ...
        @php
          ...
          foreach ($item->informed as $informed) {
            if( $informed->volume_20ter_id!=$volume_20ter->id && $informed->volume_20ter_id < $volume_20ter->id  ){
        ...
        <tr>
          ...
          <td>
            <div class="checkbox checkbox-success">
              <input id="detail-2" name="detail_id[{{ $item->id }}]" class="detail-item" type="checkbox" value="1" @if(array_key_exists($item->id, $volume_20ter_details)) checked="checked" @endif>
              <label for="detail-2"> ผลิต </label>
            </div>
          </td>
          <td>
            <input class="form-control quantity" name="quantity[{{ $item->id }}]" type="number" @if(array_key_exists($item->id, $volume_20ter_details)) value="{{ $volume_20ter_details[$item->id] }}" @else disabled="disabled" @endif step="0.01" max="9999999999.99">
          </td>
...
<div class="form-group">
  <div class="col-md-offset-4 col-md-4">
    ...
    <a class="btn btn-default" href="{{url('/esurv/volume_20ter')}}">
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
     check_max_size_file();
    var applicant_20ters = JSON.parse('{!! json_encode($applicant_20ters->toArray()) !!}'); //ข้อมูลคำขอทำ

    //เมื่อเลือกเลขที่อ้างอิง
    $('#applicant_20ter_id').change(function(event) {
      ...
      if(id!=''){
        ...
        //แสดงข้อมูลคำขอ
        $(applicant_20ters).each(function(index, item) {
          if(item.id==id){
        ...
        $.ajax("{{ url('esurv/applicant_20ter/detail-item') }}/" + id)
         .done(function(data) {
            ...
            if(data.length>0){
              ...
              $(data).each(function(index, item) {
                ...
                $(item.informed).each(function(index, informed) {
                  if(informed.volume_20ter_id!={{(int)@$volume_20ter->id}}){
