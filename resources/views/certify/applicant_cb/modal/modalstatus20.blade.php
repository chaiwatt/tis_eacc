...
<div class="modal fade text-left" id="PayIn2Modal{{$id}}" tabindex="-1" role="dialog" aria-labelledby="addBrand">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            ...
            @if(!is_null($PayIn2))
            {!! Form::open(['url' => 'certify/applicant-cb/update/pay-in2/'.$PayIn2->id, 'class' => 'form-horizontal app_form_pay_in2','id'=>'app_certi_form', 'files' => true]) !!}
            @php
                ...
                <div class="container-fluid">
                    <h3 class="text-center"><span >{{ HP::formatDateThai($PayIn2->report_date) ?? '-' }}</span></h3>
                        ...
                        <p style="text-indent: 50px;">ตามที่  {{ $name }} ได้แจ้งขอรับบริการการตรวจประเมินความสามารถ 
                            ตามมาตรฐาน มอก. {{ !is_null($std_name)?$std_name:'-' }}  ลงรับวันที่  {{ !empty($save_date) ?  HP::formatDateThai($save_date) : '-' }} </span>นั้น
                        </p>
                        ...
                        @if ($PayIn2->conditional_type == 1) <!--  เรียกเก็บค่าธรรมเนียม -->
                            ...
                            <p>ใบแจ้งหนี้ค่าธรรมเนียม :
                                    @if(!is_null($files1))
                                    <a href="{{url('certify/check/file_cb_client/'.$files1.'/'.( !empty($file_client_name) ? $file_client_name :   basename($files1) ))}}" target="_blank">
