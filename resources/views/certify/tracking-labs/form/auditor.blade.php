...
{{-- {{$tracking->AuditorsManyBy->count()}}
{{$tracking->fullyApprovedAuditorNoCancels->count()}} --}}
            @if ($tracking->AuditorsManyBy->count() > $tracking->fullyApprovedAuditorNoCancels->count())
            <span class="text-danger">(รอดำเนินการ {{$tracking->AuditorsManyBy->count() - $tracking->fullyApprovedAuditorNoCancels->count()}} คณะ)</span> 
            @endif
...
{{-- {{count($approvedAuditors)}} --}}
 @foreach($tracking->fullyApprovedAuditorNoCancels as $key => $item)
