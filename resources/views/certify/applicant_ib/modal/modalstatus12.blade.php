...
<div class="modal fade" id="PayIn1Modal" tabindex="-1" role="dialog" aria-labelledby="PayIn1Modal" aria-hidden="true">
 ...
 <div class="modal-content">
     ...
     {{-- readonly --}}
 {!! Form::open(['url' => 'certify/applicant-ib/pay-in/'.$id, 'class' => 'form-horizontal pay_in1_form','id'=>'app_certi_form', 'files' => true]) !!}
 @if($status == 'false')
     <div class="modal-body">
        <div class="container-fluid">
            <p>เรียน <span class="nameCertiLab">{{ $name }}</span> </p>
            <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ตามที่ท่านได้ยื่นคำขอรับใบรับรองหน่วยตรวจ หมายเลขคำขอที่
           ...
           @if(count($payin1) > 0)
            @foreach($payin1 as $key => $item)
              @if($item->state == 1)
                <p>
                    {{ ( $i++ ).'.' }}
                    <span  style="color:blue"> {{$item->CertiIBAuditorsTo->auditor ?? null }}</span>   
