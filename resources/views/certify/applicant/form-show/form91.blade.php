            ...
            <tbody id="labCalibrate_tbody">
                @foreach($certi_lab->certi_lab_calibrate as $lab_calibrate)
                    <tr>
                        ...
                        <td class="text-center" style="vertical-align:top">{{$lab_calibrate->getBranch()->title ?? '-'}}</td>
                        <td class="text-center" style="vertical-align:top">{{$lab_calibrate->getGroup()->title ?? '-'}}</td>
                        <?php
                            $checking_list = '';
                            $string_type = '';
                            $limit_show = '';
                            ...
                            <?php
                                if ($item->item_id){
                                    ...
                                    if ($item->item_id != $checking_list){
                                        ...
                                        $string_type .= $in_item_list ? "<p style='font-weight: bold'>- ".$in_item_list->title ?? ' '."</p>" : "<p style='font-weight: bold'>-</p>";
                                        $limit_show .= "<p>&nbsp;</p>";
                                        ...
                                        if ($formula_col->count() > 0){
                                            foreach ($formula_col as $formula){
                                                $formula_show .= "<p>&nbsp;".$formula->formula ?? ' '."</p>";
                                            }
                                        }
                                    }
                                    $size = $formula_col->count();
                                    ...
                                    if ($detail_arr->count() > 0){
                                        foreach ($detail_arr as $deli){
                                            $string_type .= "<p>&nbsp;&nbsp;• ".$deli->detail ?? ' '."</p>";
                                            $limit_show .= "<p>&nbsp;&nbsp;• ".$deli->limit ?? ' '."</p>";

                                            if ($size <= $detail_arr->count()){
                                                $formula_show .= "<p>&nbsp;</p>";
                                                $size++;
                                            }
                                        }
