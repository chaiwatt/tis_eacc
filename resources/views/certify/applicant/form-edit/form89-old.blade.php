...
@if ($certi_lab)
    @if($certi_lab->employee->count() > 0)
        <button class="btn btn-success btn-sm pull-right m-b-10 m-l-5" id="work-add" type="button">
            <i class="icon-plus"></i> เพิ่ม
        </button>
        <div id="work_responsibility_container">
            @foreach ($certi_lab->employee as $employee)
                <?php $key++ ?>
                <div class="work-box">
...
@push('js')
    <script>
        $(document).ready(function () {
            ...
            let nameResponsibility_val = '{!! $certi_lab->employee->count() !!}';
            if (nameResponsibility_val > 0){
                $('#nameResponsibility').val(nameResponsibility_val);
            }else{
