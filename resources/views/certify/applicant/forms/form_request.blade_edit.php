...
<style>
    ...
    .disabled {
        pointer-events: none; /* หยุดการโต้ตอบ */
        opacity: 0.5; /* ลดความโปร่งใส */
    }
</style>
...
<script>
    ...
    $(document).ready(function () {
        ...
        const urlType = "{{ $urlType }}"; // Blade แทรกค่า $urlType

        if (urlType === 'edit') {
            $("#page_wrapper").find("input, select, textarea, button").prop("readonly", true);

            $("#page_wrapper").css({
                "pointer-events": "none", // ปิดการโต้ตอบ
                "opacity": "0.7"         // ลดความโปร่งใส
            });
        }
