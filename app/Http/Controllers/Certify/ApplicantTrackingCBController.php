...
class ApplicantTrackingCBController extends Controller
{
    ...
    public function index(Request $request)
    {
         ...
         $model = str_slug('view-tracking-cb','-');
        ...
        if(!empty($data_session)){
            if(HP::CheckPermission($model)){
                ...
                return view('certify.tracking-cb.index', compact('filter','query'));
            }
            abort(403);
        }else{
    ...
    public function auditor(Request $request,$id)
    {
        ...
        if(!empty($data_session)){
          if(HP::CheckPermission($model)){
             ...
             return view('certify.tracking-cb.form.auditor', compact('tracking'));                    
           }
            abort(403);
         }else{
            return  redirect(HP::DomainTisiSso());  
...
// try{
      ...
      if(!is_null($tracking)){
               ...
               if(!is_null($auditors)){
                ...
                $history = TrackingHistory::where('tracking_id',$tracking->id)
                 ...
                 if($auditors->status == 1){
                      ...
                      $payin->certificate_type  =  1;
                      $payin->reference_refno   =  $tracking->reference_refno;
                      $payin->ref_table         =  (new CertiCBExport)->getTable();
        ...
        if(count($tracking->AssignEmails) > 0  && count($authorities) > 0 && !empty($tracking->certificate_export_to->CertiCbTo)){  //  ส่ง E-mail ผก.  
              ...
              $data_app =  ['title'         =>  '',
                            ...
                            'url'          => $url.'certificate/tracking-cb/'.$tracking->id .'/edit',
                            'email_cc'     => (count($certi->DataEmailDirectorCBCC) > 0 ) ? $certi->DataEmailDirectorCBCC : []
                           ];
                ...
                $log_email =  HP::getInsertCertifyLogEmail(!empty($auditors->tracking_to->reference_refno)? $auditors->tracking_to->reference_refno:null,   
                                                            ...
                                                            (count($certi->DataEmailDirectorCBCC) > 0 ) ? implode(",",$certi->DataEmailDirectorCBCC) : null,
                                                             null
                                                          );
 ...
 ;
             ...
             if(!empty($tracking->certificate_export_to->CertiCbTo->CertiEmailLt)  && count($authorities) > 0){  //  ส่ง E-mail เจ้าหน้าที ลท. CB   + เจ้าหน้าที่รับผิดชอบ
                 $certi =      $tracking->certificate_export_to->CertiCbTo;
                $data_app =  [   'title'        =>  '',
                                'email'        => $certi->email,
                                'auditors'     => $auditors,
      ...
      if($request->previousUrl){
          ...
          return redirect('certify/tracking-cb')->with('message', 'เรียบร้อยแล้ว!');
      }
//   } catch (\Exception $e) {
  ...
  public function pay_in1(Request $request,$id)
  {
      ...
      if(!empty($data_session)){
        if(HP::CheckPermission($model)){
           ...
           return view('certify.tracking-cb.form.pay_in1', compact('pay_in'));                    
         }
          abort(403);
       }else{
          return  redirect(HP::DomainTisiSso());  
        ...
        if(!is_null($pay_in) && isset($request->attachs_file)  && $request->hasFile('attachs_file')){
          ...
          // สถานะ แต่งตั้งคณะกรรมการ
           ...
           //  Log
            ...
            $export =   CertiCBExport::findOrFail($pay_in->ref_id);
            if(!empty($export)  && !empty($export->CertiCbTo->CertiEmailLt)){
        
                    $certi  =  $export->CertiCbTo;
                    ...
                    $data_app =  [ 'certi'      => $certi  ,
                                    ...
                                    'email'     => $certi->email,
                                    'pay_in'    => $pay_in,
                                    'url'       => $url.'/certificate/tracking-cb/Pay_In1/'.$pay_in->id,
                                    'email_cc'  =>  (count($certi->DataEmailDirectorCBCC) > 0 ) ? $certi->DataEmailDirectorCBCC :  []
                                ];
  ...
  public function assessment(Request $request,$id)
  {
      ...
      if(!empty($data_session)){
        if(HP::CheckPermission($model)){
           ...
           return view('certify.tracking-cb.form.assessment', compact('assessment'));                    
         }
          abort(403);
       }else{
          return  redirect(HP::DomainTisiSso());  
...
// try{
  ...
  if(!is_null($tracking)){
           ...
           // ส่ง Email เจ้าหน้าที่มอบหมาย
            if(!empty($tracking->certificate_export_to->CertiCbTo->email)  && (count($tracking->AssignEmails) > 0 )){
                ...
                $certi  =  $tracking->certificate_export_to->CertiCbTo;
                 ...
                 $data_app =  [
                                ...
                                'certi'     => $certi,
                                'url'       => $url.'certificate/assessment-cb/'.$assessment->id.'/edit',
                                'email_cc'  => (count($certi->DataEmailDirectorCBCC) > 0 ) ? $certi->DataEmailDirectorCBCC :  []
                              ];
  ...
  public function evaluation(Request $request,$id)
  {
      ...
      if(!empty($data_session)){
        if(HP::CheckPermission($model)){
           ...
           return view('certify.tracking-cb.form.evaluation', compact('evaluation'));                    
         }
          abort(403);
       }else{
          return  redirect(HP::DomainTisiSso());  
 ...
 // try{  
   ...
   if(!is_null($tracking)){
            if($inspection->status  == 1){
                ...
                $review->ref_table           = (new CertiCBExport)->getTable();
                $review->certificate_type    = 1;
        ...
        //Mail
        if(!empty($tracking->certificate_export_to->CertiCbTo) && (count($tracking->AssignEmails) > 0 )){
    ...
    if($report->status_confirm == 1){
        ...
        if(!is_null($tracking)){
           ...
           $history = TrackingHistory::where('tracking_id',$tracking->id)
            ...
            //Mail
            if(!empty($tracking->certificate_export_to->CertiCbTo)){
                $config = HP::getConfig();
                $url  =   !empty($config->url_center) ? $config->url_center : url('');    
                $certi  =  $tracking->certificate_export_to->CertiCbTo;
                ...
                $mail = new ReportMail(['certi'      => $certi ?? '-',
                                            ...
                                            'report'      => $report,
                                            'url'         => $url.'/certificate/tracking-cb/'.$tracking->id .'/edit',
                                            'email_cc'    =>  (count($certi->DataEmailDirectorCBCC) > 0 ) ? $certi->DataEmailDirectorCBCC : 'lab1@tisi.mail.go.th'
                                        ]);
        ...
        if(!empty($tracking->certificate_export_to->CertiCbTo)){
            ...
            $mail = new PayInTwoMail(['certi'        => $certi ?? '-',
                                      'export'       =>  $tracking->certificate_export_to ,
                                       ...
                                       'pay_in'      => $pay_in,
                                       'url'         => $url.'/certificate/tracking-cb/'.$tracking->id .'/edit',
                                       'email_cc'    =>  (count($certi->DataEmailDirectorCBCC) > 0 ) ? $certi->DataEmailDirectorCBCC : 'cb@tisi.mail.go.th'
                                    ]);
            if(count($tracking->AssignEmails) > 0 ){
                    Mail::to($tracking->AssignEmails)->send($mail);
            }
