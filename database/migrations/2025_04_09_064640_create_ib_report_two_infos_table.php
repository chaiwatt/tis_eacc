...
class CreateIbReportTwoInfosTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('ib_report_two_infos', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedInteger('ib_assessment_id')->nullable(); 
            $table->foreign('ib_assessment_id')->references('id')->on('app_certi_ib_assessment')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('ib_report_two_infos');
    }
}
